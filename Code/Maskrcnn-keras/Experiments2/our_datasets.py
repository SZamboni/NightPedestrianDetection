...
def get_Image(image_path,preproType):
        
    if preproType == 'Original':
        ...
        img_gamma = agcwd(RGB_img, w=0.5)
        return img_gamma

    elif preproType == 'CLAHE':
        img = io.imread(image_path)
        img_clahe = exposure.equalize_adapthist(img, clip_limit=0.05)
        return img_clahe

    elif preproType == 'MSRCP':
        original_img = cv2.imread(image_path)
        RGB_img = cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB)
        img_msrcp = MSRCP(RGB_img)
        return img_msrcp

    elif preproType == 'Ying_CAIP':
        original_img = cv2.imread(image_path)
        RGB_img = cv2.cvtColor(original_img, cv2.COLOR_BGR2RGB)
        img_ying = Ying_2017_CAIP(RGB_img)
        return img_ying
    else:
        print('WRONG PREPROCESSING')
        raise ValueError('WRONG PREPROCESSING PASSED')
...
class OurTestDataset(Dataset):
    ...
    def load_dataset(self, images_dir, annFile, preproType='Original'):
    ...
    def load_image(self, image_id):
        ...
        img = get_Image(img_path,self.preproType)
        return img
...
class OurTrainDataset(Dataset):
    ...
    # load the dataset definitions
    def load_dataset(self, images_dir, annFile, ped_ratio = 1,preproType='Original'):
        self.preproType = preproType
        self.cocovar = COCO(annFile)
        self.imgIds = list(sorted(self.cocovar.imgs.keys()))
        self.images_dir = images_dir
